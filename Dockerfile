...
COPY --link ./ $GOPATH/pkg/mod/github.com/minho-bharvest/coredns-dockerdiscovery
RUN sed -i "s/^#.*//g; /^$/d; $PLUGIN_PRIO i docker:dockerdiscovery" plugin.cfg \
    && go mod edit -replace\
    dockerdiscovery=$GOPATH/pkg/mod/github.com/minho-bharvest/coredns-dockerdiscovery\
